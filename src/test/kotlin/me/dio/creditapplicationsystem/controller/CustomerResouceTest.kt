...
class CustomerResouceTest {
    ...
    fun `should create a customer and return 201 status`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("giovanna@email.com"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("1000.0"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("000000"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.street").value("Rua da Gi, 123"))
    ...
    fun `should not save a customer with same CPF and return 409 status`() {
        ...
        mockMvc.perform(
            MockMvcRequestBuilders.post(URL)
                .contentType(MediaType.APPLICATION_JSON)
                .content(valueAsString)).andExpect(
    ...
    fun `should find customer by id and return 200 status`() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get(
            "$URL/${customer.id}").accept(MediaType.APPLICATION_JSON))
    ...
    fun `should not found customer with invalid id and return 400 status`() {
        ...
        //then
        mockMvc.perform(MockMvcRequestBuilders.get(
            "$URL/$invalidId").accept(MediaType.APPLICATION_JSON))
            .andExpect(MockMvcResultMatchers.status().isBadRequest)
            .andExpect(MockMvcResultMatchers.jsonPath("$.title").value("Bad Request! Consult the documentation"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.timestamp").exists())
            .andExpect(MockMvcResultMatchers.jsonPath("$.status").value(400))
    ...
    fun `should update a customer and return 200 status`() {
        ...
        mockMvc.perform(
            MockMvcRequestBuilders.patch("$URL?customerId=${customer.id}")
                ...
                .content(valueAsString))
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("GioUpdate"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.lastName").value("CostaUpdate"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("28475934625"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("giovanna@email.com"))
    ...
    private fun builderCustomerDto(
        ...
        email: String = "giovanna@email.com",
        income: BigDecimal = BigDecimal.valueOf(1000.0),
        password: String = "1234",
        zipCode: String = "000000",
        street: String = "Rua da Gi, 123",
    ) = CustomerDto(
        firstName = firstName,
        lastName = lastName,
        cpf = cpf,
